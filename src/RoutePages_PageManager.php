...
class RoutePages_PageManager {
	 ...
	 * returned by the RoutePages::SHOULD_GENERATE_ROUTE_POSTS
	 * filter.
	 */
	...
	protected function shouldGenerateRoutePages() {
		 ...
		 *
		 *     add_filter(RoutePages::SHOULD_GENERATE_ROUTE_POSTS, '__return_false');
		 */
		if ( ! $this->f->apply_filters( RoutePages::SHOULD_GENERATE_ROUTE_POSTS, null ) ) {
			return false;
		}
	...
	public function generateRoutePages() {
		...
		if ( is_array( $persistedRoutesMeta ) ) {
			foreach ( $persistedRoutesMeta as $routeMetaKey => $routeMeta ) {
				...
				// strip lead ^ and trailing $ from permalink
				$post_title = rtrim( ltrim( $routeMeta['permalink'], '^' ), '$' );
				// if the permalink contains path vars then do not generate the page
				if ( ! preg_match( "/^[\\w\\d-_\\/]+$/um", $post_title ) ) {
					continue;
				}
